...
def evaluate_snapshot(input_snapshot_path, test_docs, output_lambda_path, output_nu_square_path, output_gamma_path, output_sigma_square_path):
    # import hybrid, monte_carlo, variational_bayes;
    lntm_inferencer = cPickle.load(open(input_snapshot_path, "rb"));
    # print 'successfully load model snapshot %s...' % (os.path.abspath(input_snapshot_path));
    
    log_likelihood, lambda_values, nu_square_values = lntm_inferencer.inference(test_docs);
