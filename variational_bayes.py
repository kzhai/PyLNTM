...
class Process_E_Step_Queue(multiprocessing.Process):
    def __init__(self,
        ...
        if self._diagonal_covariance_matrix:
            ...
            (self._beta_lambda, self._log_zeta_beta, self._alpha_mu, self._alpha_sigma, self._alpha_sigma_inv) = model_parameters;
    ...
    def run(self):
        ...
        while not self._task_queue.empty():
            ...
            if self._diagonal_covariance_matrix:
                ...
                document_log_likelihood -= 0.5 * numpy.dot(numpy.dot((self._alpha_mu - doc_lambda[numpy.newaxis, :]), self._alpha_sigma_inv), (self._alpha_mu - doc_lambda[numpy.newaxis, :]).T);
            ...
            if self._result_sufficient_statistics_queue == None:
                ...
                words_log_likelihood += numpy.sum(numpy.exp(log_phi + numpy.log(term_counts)) * self._E_log_prob_eta[:, term_ids]);
...
class Process_M_Step_Queue(multiprocessing.Process):
    ...
    def run(self):
        ...
        while not self._task_queue.empty():
            try:
                (topic_id, topic_lambda, topic_nu_square, topic_phi_sufficient_statistics) = self._task_queue.get_nowait();
            ...
            for local_parameter_iteration_index in xrange(self._parameter_iteration):
                ...
                arguments = (topic_nu_square, topic_zeta_factor, topic_phi_sufficient_statistics, topic_phi_sufficient_statistics_sum);
            ...
            if self._diagonal_covariance_matrix:
                ...
                topic_log_likelihood += -0.5 * numpy.dot(numpy.dot((topic_lambda[numpy.newaxis, :] - self._beta_mu), self._beta_sigma_inv), (topic_lambda[numpy.newaxis, :] - self._beta_mu).T);
...
class VariationalBayes(Inferencer):
    ...
    def import_word_correlation(self, word_correlation_path):
        ...
        for line in input_stream:
            ...
            if (word_a_index, word_b_index) not in self._word_correlations:
                ...
                print "warning: duplicate word correlation entry for (%s, %s) found, aggregating the correlations..." % (tokens[0], tokens[1]);
                self._word_correlations[(word_a_index, word_b_index)] += correlation;
    ...
    def e_step_process_queue(self,
        ...
        if self._diagonal_covariance_matrix:
            ...
            e_step_parameters = (self._beta_lambda, log_zeta_beta, self._alpha_mu, self._alpha_sigma, self._alpha_sigma_inv);
        ...
        # initialize a D-by-K matrix lambda and nu_square values
        lambda_values = numpy.zeros((number_of_documents, self._number_of_topics))  # + self._alpha_mu[numpy.newaxis, :];
        nu_square_values = numpy.zeros((number_of_documents, self._number_of_topics))  # + self._alpha_sigma[numpy.newaxis, :];
    ...
    def e_step(self,
        ...
        # initialize a D-by-K matrix lambda and nu_square values
        alpha_lambda_values = numpy.zeros((number_of_documents, self._number_of_topics))  # + self._alpha_mu[numpy.newaxis, :];
        alpha_nu_square_values = numpy.zeros((number_of_documents, self._number_of_topics))  # + self._alpha_sigma[numpy.newaxis, :];
        ...
        for doc_id in numpy.random.permutation(number_of_documents):
            ...
            if parsed_corpus != None:
                ...
                words_log_likelihood += numpy.sum(numpy.exp(log_phi + numpy.log(term_counts)) * (self._beta_lambda[:, term_ids] - log_sum_exp_beta_lambda));
    ...
    def f_doc_lambda(self, doc_lambda, *args):
        ...
        if self._diagonal_covariance_matrix:
            ...
            assert mean_adjustment.shape == (1, self._number_of_topics), (doc_lambda.shape, mean_adjustment.shape, self._alpha_mu.shape)
            function_doc_lambda += -0.5 * numpy.dot(numpy.dot(mean_adjustment, self._alpha_sigma_inv), mean_adjustment.T)
    ...
    def f_prime_doc_lambda(self, doc_lambda, *args):
        ...
        if self._diagonal_covariance_matrix:
            ...
            function_prime_doc_lambda = numpy.dot((self._alpha_mu - doc_lambda[numpy.newaxis, :]), self._alpha_sigma_inv)[0, :]
    ...
    def f_hessian_direction_doc_lambda(self, doc_lambda, direction_vector, *args):
        ...
        log_exp_over_doc_zeta_a = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - direction_vector[:, numpy.newaxis] * self._hessian_direction_approximation_epsilon - 0.5 * doc_nu_square[:, numpy.newaxis], axis=1)
        log_exp_over_doc_zeta_b = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - 0.5 * doc_nu_square[:, numpy.newaxis], axis=1)
        ...
        function_hessian_direction_doc_lambda = total_word_count * numpy.exp(-log_exp_over_doc_zeta_b) * (1 - numpy.exp(log_exp_over_doc_zeta_b - log_exp_over_doc_zeta_a))
        
        if self._diagonal_covariance_matrix:
            function_hessian_direction_doc_lambda += -direction_vector * self._hessian_direction_approximation_epsilon / self._alpha_sigma;
        else:
            function_hessian_direction_doc_lambda += -numpy.dot(direction_vector[numpy.newaxis, :] * self._hessian_direction_approximation_epsilon, self._alpha_sigma_inv)[0, :]
    ...
    def f_hessian_direction_doc_nu_square(self, doc_nu_square, direction_vector, *args):
        ...
        log_exp_over_doc_zeta_a = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - 0.5 * (doc_nu_square[:, numpy.newaxis] + direction_vector[:, numpy.newaxis] * self._hessian_direction_approximation_epsilon), axis=1)
        log_exp_over_doc_zeta_b = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - 0.5 * doc_nu_square[:, numpy.newaxis], axis=1)
        ...
        function_hessian_direction_doc_nu_square = total_word_count * numpy.exp(-log_exp_over_doc_zeta_b) * (1 - numpy.exp(log_exp_over_doc_zeta_b - log_exp_over_doc_zeta_a))
        
        function_hessian_direction_doc_nu_square += 0.5 / (doc_nu_square + self._hessian_direction_approximation_epsilon * direction_vector);
    ...
    def f_prime_log_doc_nu_square(self, log_doc_nu_square, *args):
        ...
        exp_over_doc_zeta = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - 0.5 * exp_log_doc_nu_square[:, numpy.newaxis], axis=1)
        exp_over_doc_zeta = numpy.exp(-exp_over_doc_zeta);
    ...
    def f_hessian_log_doc_nu_square(self, log_doc_nu_square, *args):
        ...
        exp_over_doc_zeta = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - 0.5 * exp_doc_log_nu_square[:, numpy.newaxis], axis=1)
        exp_over_doc_zeta = numpy.exp(-exp_over_doc_zeta);
        ...
        function_hessian_log_doc_nu_square -= 0.5 * total_word_count * exp_over_doc_zeta * exp_doc_log_nu_square * (1 + 0.5 * exp_doc_log_nu_square);
    ...
    def f_hessian_direction_log_doc_nu_square(self, log_doc_nu_square, direction_vector, *args):
        ...
        exp_log_doc_nu_square = numpy.exp(log_doc_nu_square);
        exp_log_doc_nu_square_epsilon_direction = numpy.exp(log_doc_nu_square + direction_vector * self._hessian_direction_approximation_epsilon);
        
        log_exp_over_doc_zeta_epsilon_direction = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - 0.5 * exp_log_doc_nu_square_epsilon_direction[:, numpy.newaxis], axis=1)
        log_exp_over_doc_zeta = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - 0.5 * exp_log_doc_nu_square[:, numpy.newaxis], axis=1)
        ...
        function_hessian_direction_log_doc_nu_square = 1 - numpy.exp(direction_vector * self._hessian_direction_approximation_epsilon - log_exp_over_doc_zeta_epsilon_direction + log_exp_over_doc_zeta)
        function_hessian_direction_log_doc_nu_square *= 0.5 * total_word_count * numpy.exp(log_doc_nu_square - log_exp_over_doc_zeta)
        
        if self._diagonal_covariance_matrix:
            function_hessian_direction_log_doc_nu_square += 0.5 * (exp_log_doc_nu_square - exp_log_doc_nu_square_epsilon_direction) / self._alpha_sigma;
        else:
            function_hessian_direction_log_doc_nu_square += 0.5 * (exp_log_doc_nu_square - exp_log_doc_nu_square_epsilon_direction) * numpy.diag(self._alpha_sigma_inv);
    ...
    def m_step_process_queue(self,
        ...
        for topic_id in xrange(self._number_of_topics):
            task_queue.put((topic_id, self._beta_lambda[topic_id, :], self._beta_nu_square[topic_id, :], phi_sufficient_statistics[topic_id, :]));
    ...
    def m_step(self,
        ...
        for topic_id in numpy.random.permutation(self._number_of_topics):
            ...
            for local_parameter_iteration_index in xrange(local_parameter_iteration):
                ...
                arguments = (topic_nu_square, topic_zeta_factor, phi_sufficient_statistics[topic_id, :], phi_sufficient_statistics_sum[topic_id]);
                topic_lambda = self.optimize_topic_lambda(topic_lambda, arguments);
    ...
    def f_topic_lambda(self, topic_lambda, *args):
        ...
        exp_over_topic_zeta = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - 0.5 * topic_nu_square[:, numpy.newaxis], axis=1)
        ...
        if self._diagonal_covariance_matrix:
            ...
            function_topic_lambda -= 0.5 * numpy.dot(numpy.dot(mean_adjustment, self._beta_sigma_inv), mean_adjustment.T);
    ...
    def f_prime_topic_lambda(self, topic_lambda, *args):
        ...
        if self._diagonal_covariance_matrix:
            ...
            function_prime_topic_lambda = numpy.dot((self._beta_mu - topic_lambda[numpy.newaxis, :]), self._beta_sigma_inv)[0, :];
    ...
    def f_hessian_direction_topic_lambda(self, topic_lambda, direction_vector, *args):
        ...
        log_exp_over_topic_zeta_a = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - direction_vector[:, numpy.newaxis] * self._hessian_direction_approximation_epsilon - 0.5 * topic_nu_square[:, numpy.newaxis], axis=1)
        log_exp_over_topic_zeta_b = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - 0.5 * topic_nu_square[:, numpy.newaxis], axis=1)
        assert log_exp_over_topic_zeta_a.shape == (self._number_of_types,)
        assert log_exp_over_topic_zeta_b.shape == (self._number_of_types,)
        
        function_hessian_direction_topic_lambda = topic_phi_sum * numpy.exp(-log_exp_over_topic_zeta_b) * (1 - numpy.exp(log_exp_over_topic_zeta_b - log_exp_over_topic_zeta_a))
        
        if self._diagonal_covariance_matrix:
            function_hessian_direction_topic_lambda = -direction_vector * self._hessian_direction_approximation_epsilon / self._beta_sigma;
        else:
            function_hessian_direction_topic_lambda += -numpy.dot(direction_vector[numpy.newaxis, :] * self._hessian_direction_approximation_epsilon, self._beta_sigma_inv)[0, :]
        
        function_hessian_direction_topic_lambda /= self._hessian_direction_approximation_epsilon;
        
        assert function_hessian_direction_topic_lambda.shape == (self._number_of_types,), function_hessian_direction_topic_lambda.shape;
    ...
    def f_hessian_direction_topic_nu_square(self, topic_nu_square, direction_vector):
        ...
        log_exp_over_topic_zeta_a = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - 0.5 * (topic_nu_square[:, numpy.newaxis] + direction_vector[:, numpy.newaxis] * self._hessian_direction_approximation_epsilon), axis=1)
        log_exp_over_topic_zeta_b = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - 0.5 * topic_nu_square[:, numpy.newaxis], axis=1)
        ...
        function_hessian_direction_topic_nu_square = topic_phi_sufficient_statistics * numpy.exp(-log_exp_over_topic_zeta_b) * (1 - numpy.exp(log_exp_over_topic_zeta_b - log_exp_over_topic_zeta_a))
        
        function_hessian_direction_topic_nu_square += 0.5 / (topic_nu_square + self._hessian_direction_approximation_epsilon * direction_vector);
    ...
    def f_prime_log_topic_nu_square(self, log_topic_nu_square, *args):
        ...
        exp_over_topic_zeta = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - 0.5 * exp_log_topic_nu_square[:, numpy.newaxis], axis=1)
        exp_over_topic_zeta = numpy.exp(-exp_over_topic_zeta);
    ...
    def f_hessian_log_topic_nu_square(self, log_topic_nu_square, *args):
        ...
        function_hessian_log_topic_nu_square -= 0.5 * topic_phi_sufficient_statistics * exp_over_topic_zeta * exp_log_topic_nu_square * (1 + 0.5 * exp_log_topic_nu_square);
    ...
    def f_hessian_direction_log_topic_nu_square(self, log_topic_nu_square, direction_vector, *args):
        ...
        exp_log_topic_nu_square = numpy.exp(log_topic_nu_square);
        exp_log_topic_nu_square_epsilon_direction = numpy.exp(log_topic_nu_square + direction_vector * self._hessian_direction_approximation_epsilon);
        
        log_exp_over_topic_zeta_epsilon_direction = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - 0.5 * exp_log_topic_nu_square_epsilon_direction[:, numpy.newaxis], axis=1)
        log_exp_over_topic_zeta = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - 0.5 * exp_log_topic_nu_square[:, numpy.newaxis], axis=1)
        
        function_hessian_direction_log_topic_nu_square = 1 - numpy.exp(direction_vector * self._hessian_direction_approximation_epsilon - log_exp_over_topic_zeta_epsilon_direction + log_exp_over_topic_zeta)
        function_hessian_direction_log_topic_nu_square *= 0.5 * topic_phi_sufficient_statistics * numpy.exp(log_topic_nu_square - log_exp_over_topic_zeta)
        
        if self._diagonal_covariance_matrix:
            function_hessian_direction_log_topic_nu_square += 0.5 * (exp_log_topic_nu_square - exp_log_topic_nu_square_epsilon_direction) / self._beta_sigma;
        else:
            function_hessian_direction_log_topic_nu_square += 0.5 * (exp_log_topic_nu_square - exp_log_topic_nu_square_epsilon_direction) * numpy.diag(self._beta_sigma_inv);
    ...
    def learning(self, number_of_processes=1):
        ...
        print "e_step and m_step of iteration %d finished in %g and %g seconds respectively with log likelihood %g" % (self._counter, clock_e_step, clock_m_step, joint_log_likelihood)
    ...
    def optimize_hyperparameters(self):
        if self._optimize_alpha_hyperparemters:
            ...
            if self._diagonal_covariance_matrix:
                self._alpha_sigma = numpy.mean(self._alpha_nu_square + (self._alpha_lambda - self._alpha_mu[numpy.newaxis, :]) ** 2, axis=0);
        ...
        if self._optimize_beta_hyperparemters:
            ...
            assert self._beta_nu_square.shape == (self._number_of_topics, self._number_of_types);
            if self._diagonal_covariance_matrix:
                self._beta_sigma = numpy.mean(self._beta_nu_square + (self._beta_lambda - self._beta_mu[numpy.newaxis, :]) ** 2, axis=0);
    ...
    def export_beta(self, exp_beta_path, top_display=-1):
        ...
        for topic_index in xrange(self._number_of_topics):
            ...
            beta_probability = numpy.exp(self._beta_lambda[topic_index, :] - scipy.misc.logsumexp(self._beta_lambda[topic_index, :]))
    ...
    def newton_method_doc_nu_square(self,
        ...
        for newton_method_iteration_index in xrange(newton_method_iteration):
            ...
            assert numpy.all(doc_nu_square > 0), (doc_nu_square, step_change, first_derivative_nu_square, second_derivative_nu_square);
    ...
    def newton_method_doc_nu_square_in_log_space(self,
        ...
        newton_method_power_index = 0;
        for newton_method_iteration_index in xrange(newton_method_iteration):
            log_exp_over_doc_zeta_combine = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - 0.5 * exp_doc_log_nu_square[:, numpy.newaxis] - doc_log_nu_square[:, numpy.newaxis], axis=1)
            ...
            second_derivative_log_nu_square += -0.5 * total_word_count * exp_over_doc_zeta_combine * (1 + 0.5 * exp_doc_log_nu_square);
    ...
    def hessian_free_doc_lambda(self,
        ...
        for hessian_free_iteration_index in xrange(hessian_free_iteration):
            delta_doc_lambda = self.conjugate_gradient_delta_doc_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count, self._number_of_topics);
    ...
    def conjugate_gradient_delta_doc_lambda(self,
        ...
        if precondition_hessian_matrix:
            hessian_lambda = self.second_derivative_doc_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count);
        ...
        r_vector = -self.first_derivative_doc_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count);
        ...
        r_vector -= self.hessian_damping_direction_approximation_doc_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count, delta_doc_lambda)
        ...
        for conjugate_gradient_iteration_index in xrange(conjugate_gradient_iteration):
            ...
            hessian_p_vector = self.hessian_damping_direction_approximation_doc_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count, p_vector);
    ...
    def first_derivative_doc_lambda(self, doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count):
        ...
        if self._diagonal_covariance_matrix:
            ...
            first_derivative_doc_lambda = numpy.dot((self._alpha_mu - doc_lambda[numpy.newaxis, :]), self._alpha_sigma_inv)[0, :]
    ...
    def hessian_direction_approximation_doc_lambda(self, doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, direction_vector, epsilon=1e-6):
        assert doc_lambda.shape == (self._number_of_topics,)
        assert doc_nu_square.shape == (self._number_of_topics,)
        assert doc_zeta_factor.shape == (self._number_of_topics, self._number_of_topics)
        assert direction_vector.shape == (self._number_of_topics,)
        
        log_exp_over_doc_zeta_a = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - direction_vector[:, numpy.newaxis] * epsilon - 0.5 * doc_nu_square[:, numpy.newaxis], axis=1)
        log_exp_over_doc_zeta_b = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - 0.5 * doc_nu_square[:, numpy.newaxis], axis=1)
        assert log_exp_over_doc_zeta_a.shape == (self._number_of_topics,)
        assert log_exp_over_doc_zeta_b.shape == (self._number_of_topics,)
        ...
        hessian_direction_doc_lambda = total_word_count * numpy.exp(-log_exp_over_doc_zeta_b) * (1 - numpy.exp(log_exp_over_doc_zeta_b - log_exp_over_doc_zeta_a))
        
        if self._diagonal_covariance_matrix:
            hessian_direction_doc_lambda = -direction_vector * epsilon / self._alpha_sigma;
        else:
            hessian_direction_doc_lambda += -numpy.dot(direction_vector[numpy.newaxis, :] * epsilon, self._alpha_sigma_inv)[0, :]
    ...
    def hessian_damping_direction_approximation_doc_lambda(self, doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count, direction_vector, damping_factor_initialization=0.1, damping_factor_iteration=10):
        damping_factor_numerator = self.function_doc_lambda(doc_lambda + direction_vector, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count);
        damping_factor_numerator -= self.function_doc_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count);
        
        hessian_direction_approximation = self.hessian_direction_approximation_doc_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, direction_vector);
        
        damping_factor_denominator_temp = self.first_derivative_doc_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count);
    ...
    def hessian_free_doc_nu_square(self,
        ...
        for hessian_free_iteration_index in xrange(hessian_free_iteration):
            delta_doc_nu_square = self.conjugate_gradient_delta_doc_nu_square(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, self._number_of_topics);
    ...
    def conjugate_gradient_delta_doc_nu_square(self,
        ...
        r_vector -= self.hessian_damping_direction_approximation_doc_nu_square(doc_lambda, doc_nu_square_copy, doc_zeta_factor, total_word_count, delta_doc_nu_square);
        ...
        for conjugate_gradient_iteration_index in xrange(conjugate_gradient_iteration):
            ...
            hessian_p_vector = self.hessian_damping_direction_approximation_doc_nu_square(doc_lambda, doc_nu_square_copy, doc_zeta_factor, total_word_count, p_vector);
    ...
    def hessian_direction_approximation_doc_nu_square(self, doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, direction_vector, epsilon=1e-6):
        assert doc_lambda.shape == (self._number_of_topics,)
        assert doc_nu_square.shape == (self._number_of_topics,)
        assert doc_zeta_factor.shape == (self._number_of_topics, self._number_of_topics)
        assert direction_vector.shape == (self._number_of_topics,)
        
        log_exp_over_doc_zeta_a = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - 0.5 * (doc_nu_square[:, numpy.newaxis] + direction_vector[:, numpy.newaxis] * epsilon), axis=1)
        log_exp_over_doc_zeta_b = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - 0.5 * doc_nu_square[:, numpy.newaxis], axis=1)
        ...
        hessian_direction_doc_nu_square = total_word_count * numpy.exp(-log_exp_over_doc_zeta_b) * (1 - numpy.exp(log_exp_over_doc_zeta_b - log_exp_over_doc_zeta_a))
    ...
    def hessian_damping_direction_approximation_doc_nu_square(self, doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, direction_vector, damping_factor_initialization=0.1, damping_factor_iteration=10):
        damping_factor_numerator = self.function_doc_nu_square(doc_lambda, doc_nu_square + direction_vector, doc_zeta_factor, total_word_count);
        damping_factor_numerator -= self.function_doc_nu_square(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count);
        
        hessian_direction_approximation = self.hessian_direction_approximation_doc_nu_square(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, direction_vector);
        
        damping_factor_denominator_temp = self.first_derivative_doc_nu_square(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count);
        assert damping_factor_denominator_temp.shape == (self._number_of_topics,);
        damping_factor_denominator_temp += 0.5 * hessian_direction_approximation;
        assert damping_factor_denominator_temp.shape == (self._number_of_topics,);
    ...
    def hessian_free_doc_nu_square_in_log_space(self,
        ...
        for hessian_free_iteration_index in xrange(hessian_free_iteration):
            delta_doc_log_nu_square = self.conjugate_gradient_delta_log_doc_nu_square(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, self._number_of_topics);
    ...
    def conjugate_gradient_delta_log_doc_nu_square(self,
        ...
        r_vector -= self.hessian_damping_direction_approximation_log_doc_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count, delta_doc_log_nu_square);
        ...
        for conjugate_gradient_iteration_index in xrange(conjugate_gradient_iteration):
            ...
            assert numpy.all(numpy.isfinite(r_vector)), (conjugate_gradient_iteration_index, r_vector, doc_nu_square, -self.first_derivative_log_doc_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count), -self.hessian_direction_approximation_log_doc_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count, delta_doc_log_nu_square))
            ...
            hessian_p_vector = self.hessian_damping_direction_approximation_log_doc_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count, p_vector);
    ...
    def hessian_direction_approximation_log_doc_nu_square(self, doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count, direction_vector, epsilon=1e-6):
        assert doc_lambda.shape == (self._number_of_topics,)
        assert doc_log_nu_square.shape == (self._number_of_topics,)
        assert doc_zeta_factor.shape == (self._number_of_topics, self._number_of_topics)
        assert direction_vector.shape == (self._number_of_topics,)
        ...
        log_exp_over_doc_zeta_epsilon_direction = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - 0.5 * exp_doc_log_nu_square_epsilon_direction[:, numpy.newaxis], axis=1)
        log_exp_over_doc_zeta = scipy.misc.logsumexp(doc_zeta_factor - doc_lambda[:, numpy.newaxis] - 0.5 * exp_doc_log_nu_square[:, numpy.newaxis], axis=1)
        ...
        hessian_direction_log_doc_nu_square = 1 - numpy.exp(direction_vector * epsilon - log_exp_over_doc_zeta_epsilon_direction + log_exp_over_doc_zeta)
        hessian_direction_log_doc_nu_square *= 0.5 * total_word_count * numpy.exp(doc_log_nu_square - log_exp_over_doc_zeta)
        
        if self._diagonal_covariance_matrix:
            hessian_direction_log_doc_nu_square += 0.5 * (exp_doc_log_nu_square - exp_doc_log_nu_square_epsilon_direction) / self._alpha_sigma;
        else:
            hessian_direction_log_doc_nu_square += 0.5 * (exp_doc_log_nu_square - exp_doc_log_nu_square_epsilon_direction) * numpy.diag(self._alpha_sigma_inv);
        
        hessian_direction_log_doc_nu_square /= epsilon
                
        return hessian_direction_log_doc_nu_square
    
    def hessian_damping_direction_approximation_log_doc_nu_square(self, doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count, direction_vector, damping_factor_initialization=0.1, damping_factor_iteration=10):
        damping_factor_numerator = self.function_log_doc_nu_square(doc_lambda, doc_log_nu_square + direction_vector, doc_zeta_factor, total_word_count);
        damping_factor_numerator -= self.function_log_doc_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count);
        
        hessian_direction_approximation = self.hessian_direction_approximation_log_doc_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count, direction_vector);
        
        damping_factor_denominator_temp = self.first_derivative_log_doc_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count);
        assert damping_factor_denominator_temp.shape == (self._number_of_topics,);
        damping_factor_denominator_temp += 0.5 * hessian_direction_approximation;
        assert damping_factor_denominator_temp.shape == (self._number_of_topics,);
    ...
    def hessian_free_topic_lambda(self,
        ...
        for hessian_free_iteration_index in xrange(hessian_free_iteration):
            delta_topic_lambda = self.conjugate_gradient_delta_topic_lambda(topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi, topic_phi_sum, self._number_of_types);
    ...
    def conjugate_gradient_delta_topic_lambda(self,
        ...
        if precondition_hessian_matrix:
            hessian_lambda = self.second_derivative_topic_lambda(topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi_sum);
            if not numpy.all(numpy.isfinite(hessian_lambda)):
                return numpy.zeros(self._number_of_types);
            M_inverse = 1.0 / numpy.diag(hessian_lambda);
        # print numpy.linalg.cond(hessian_lambda), ">>>", numpy.linalg.cond(numpy.dot(numpy.diag(1.0/numpy.diag(hessian_lambda)), hessian_lambda)), ">>>", numpy.linalg.cond(numpy.dot(numpy.linalg.cholesky(hessian_lambda), hessian_lambda));            
        
        r_vector = -self.first_derivative_topic_lambda(topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi, topic_phi_sum);
        r_vector -= self.hessian_damping_direction_approximation_topic_lambda(topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi, topic_phi_sum, delta_topic_lambda)
        ...
        for conjugate_gradient_iteration_index in xrange(conjugate_gradient_iteration):
            hessian_p_vector = self.hessian_damping_direction_approximation_topic_lambda(topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi, topic_phi_sum, p_vector);
            if not numpy.all(numpy.isfinite(hessian_p_vector)):
                break;
    ...
    def first_derivative_topic_lambda(self,
        ...
        if self._diagonal_covariance_matrix:
            ...
            first_derivative_topic_lambda = numpy.dot((self._beta_mu - topic_lambda[numpy.newaxis, :]), self._beta_sigma_inv)[0, :];
    ...
    def hessian_direction_approximation_topic_lambda(self,
        ...
        log_exp_over_topic_zeta_a = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - direction_vector[:, numpy.newaxis] * epsilon - 0.5 * topic_nu_square[:, numpy.newaxis], axis=1)
        log_exp_over_topic_zeta_b = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - 0.5 * topic_nu_square[:, numpy.newaxis], axis=1)
        assert log_exp_over_topic_zeta_a.shape == (self._number_of_types,)
        assert log_exp_over_topic_zeta_b.shape == (self._number_of_types,)
        
        hessian_direction_topic_lambda = topic_phi_sum * numpy.exp(-log_exp_over_topic_zeta_b) * (1 - numpy.exp(log_exp_over_topic_zeta_b - log_exp_over_topic_zeta_a))
        
        if self._diagonal_covariance_matrix:
            hessian_direction_topic_lambda = -direction_vector * epsilon / self._beta_sigma;
        else:
            hessian_direction_topic_lambda += -numpy.dot(direction_vector[numpy.newaxis, :] * epsilon, self._beta_sigma_inv)[0, :]
    ...
    def hessian_damping_direction_approximation_topic_lambda(self,
        ...
        damping_factor_numerator = self.function_topic_lambda(topic_lambda + direction_vector, topic_nu_square, topic_zeta_factor, topic_phi, topic_phi_sum);
        damping_factor_numerator -= self.function_topic_lambda(topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi, topic_phi_sum);
        
        hessian_direction_approximation = self.hessian_direction_approximation_topic_lambda(topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi_sum, direction_vector);
        
        damping_factor_denominator_temp = self.first_derivative_topic_lambda(topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi, topic_phi_sum);
    ...
    def hessian_free_topic_nu_square(self,
        ...
        for hessian_free_iteration_index in xrange(hessian_free_iteration):
            delta_topic_nu_square = self.conjugate_gradient_delta_topic_nu_square(topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi_sufficient_statistics, self._number_of_types);
    ...
    def conjugate_gradient_delta_topic_nu_square(self,
        ...
        r_vector = -self.first_derivative_topic_nu_square(topic_lambda, topic_nu_square_copy, topic_zeta_factor, topic_phi_sufficient_statistics);
        ...
        r_vector -= self.hessian_damping_direction_approximation_topic_nu_square(topic_lambda, topic_nu_square_copy, topic_zeta_factor, topic_phi_sufficient_statistics, delta_topic_nu_square);
        ...
        for conjugate_gradient_iteration_index in xrange(conjugate_gradient_iteration):
            ...
            hessian_p_vector = self.hessian_damping_direction_approximation_topic_nu_square(topic_lambda, topic_nu_square_copy, topic_zeta_factor, topic_phi_sufficient_statistics, p_vector);
            assert not numpy.any(numpy.isnan(hessian_p_vector))
    ...
    def function_topic_nu_square(self, topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi_sufficient_statistics_sum):
        assert topic_zeta_factor.shape == (self._number_of_types, self._number_of_types);
    ...
    def first_derivative_topic_nu_square(self, topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi_sufficient_statistics):
        assert topic_zeta_factor.shape == (self._number_of_types, self._number_of_types);
    ...
    def hessian_direction_approximation_topic_nu_square(self, topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi_sufficient_statistics, direction_vector, epsilon=1e-6):
        assert topic_lambda.shape == (self._number_of_types,)
        assert topic_nu_square.shape == (self._number_of_types,)
        assert topic_zeta_factor.shape == (self._number_of_types, self._number_of_types)
        assert direction_vector.shape == (self._number_of_types,)
        
        log_exp_over_topic_zeta_a = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - 0.5 * (topic_nu_square[:, numpy.newaxis] + direction_vector[:, numpy.newaxis] * epsilon), axis=1)
        log_exp_over_topic_zeta_b = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - 0.5 * topic_nu_square[:, numpy.newaxis], axis=1)
        ...
        hessian_direction_topic_nu_square = topic_phi_sufficient_statistics * numpy.exp(-log_exp_over_topic_zeta_b) * (1 - numpy.exp(log_exp_over_topic_zeta_b - log_exp_over_topic_zeta_a))
    ...
    def hessian_damping_direction_approximation_topic_nu_square(self, topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi_sufficient_statistics, direction_vector, damping_factor_initialization=0.1, damping_factor_iteration=10):
        damping_factor_numerator = self.function_topic_nu_square(topic_lambda, topic_nu_square + direction_vector, topic_zeta_factor, topic_phi_sufficient_statistics);
        damping_factor_numerator -= self.function_topic_nu_square(topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi_sufficient_statistics);
        
        hessian_direction_approximation = self.hessian_direction_approximation_topic_nu_square(topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi_sufficient_statistics, direction_vector);
        
        damping_factor_denominator_temp = self.first_derivative_topic_nu_square(topic_lambda, topic_nu_square, topic_zeta_factor, topic_phi_sufficient_statistics);
        assert damping_factor_denominator_temp.shape == (self._number_of_topics,);
        damping_factor_denominator_temp += 0.5 * hessian_direction_approximation;
        assert damping_factor_denominator_temp.shape == (self._number_of_topics,);
    ...
    def newton_method_topic_lambda(self,
        ...
        for newton_method_iteration_index in xrange(newton_method_iteration):
            ...
            if self._diagonal_covariance_matrix:
                ...
                first_derivative_lambda = numpy.dot((self._beta_mu - topic_lambda[numpy.newaxis, :]), self._beta_sigma_inv);
    ...
    def newton_method_topic_nu_square(self,
        ...
        for newton_method_iteration_index in xrange(newton_method_iteration):
            ...
            assert numpy.all(topic_nu_square > 0), (topic_nu_square, step_change, first_derivative_nu_square, second_derivative_nu_square);
    ...
    def newton_method_topic_nu_square_in_log_space(self,
        ...
        newton_method_power_index = 0;
        for newton_method_iteration_index in xrange(newton_method_iteration):
            log_exp_over_topic_zeta_combine = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - 0.5 * exp_topic_log_nu_square[:, numpy.newaxis] - topic_log_nu_square[:, numpy.newaxis], axis=1)
            ...
            second_derivative_log_nu_square += -0.5 * topic_phi_sum * exp_over_topic_zeta_combine * (1 + 0.5 * exp_topic_log_nu_square);
    ...
    def function_log_topic_nu_square(self, topic_lambda, topic_log_nu_square, topic_zeta_factor, topic_phi_sufficient_statistics):
        return self.function_topic_nu_square(topic_lambda, numpy.exp(topic_log_nu_square), topic_zeta_factor, topic_phi_sufficient_statistics);

    def first_derivative_log_topic_nu_square(self, topic_lambda, topic_log_nu_square, topic_zeta_factor, topic_phi_sufficient_statistics):
        ...
        exp_over_doc_zeta = scipy.misc.logsumexp(topic_zeta_factor - topic_lambda[:, numpy.newaxis] - 0.5 * exp_doc_log_nu_square[:, numpy.newaxis], axis=1)
        exp_over_doc_zeta = numpy.exp(-exp_over_doc_zeta);
